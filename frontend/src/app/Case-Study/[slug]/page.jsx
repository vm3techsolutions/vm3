...
export default async function page({ params }) {
  ...
  return (
    <section className="py-12 px-6 max-w-5xl mx-auto">
        <h1 className="text-3xl text-center font-bold mb-4 pt-15">{caseStudy.title}</h1>
      {caseStudy.image && (
      ...
      {caseStudy.subtitle && (
        <p className="text-gray-600 mb-4">
          {Array.isArray(caseStudy.subtitle) ? caseStudy.subtitle.join(", ") : caseStudy.subtitle}
        </p>
      )}
      ...
      {caseStudy.challenges && (
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside">
            {caseStudy.challenges.map((ch, i) => <li key={i}>{ch}</li>)}
          </ul>
        </div>
      )}
      ...
      {caseStudy.keyFeatures && (
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside">
            {caseStudy.keyFeatures.map((kf, i) => <li key={i}>{kf}</li>)}
          </ul>
        </div>
      )}
